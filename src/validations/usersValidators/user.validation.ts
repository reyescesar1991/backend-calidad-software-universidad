...
export const updateUserSchemaZod = z.object({
    ...
    name: z.string().min(1, "Nombre del usuario es requerido").optional(),
    lastName: z.string().min(1, "Apellido del usuario es requerido").optional(),
    codeCountry: z.string().min(1, "El código de país es requerido").optional(),
    phoneNumber: z.string().regex(/^(0412|0416|0424|0414)\d{7}$/, { message: 'Formato de telefono erroneo' }).optional(),
    email: z.string().email().optional(),
    password: z.string().regex(/^(?=.*[a-z])(?=.*[A-Z])(?=.*[0-9])(?=.*[!@#$%^&*()_+{}\[\]:;<>,.?~\\/-]).{12,}$/, { message: "La contraseña debe tener al menos 12 caracteres y contener al menos una mayúscula, una minúscula, un número y un carácter especial." }).optional(),
