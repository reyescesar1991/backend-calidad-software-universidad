...
const runTestFindProductByBarcode = async () => {
    try {
        ...
        const productBarcode = "12345678";
        const productService = tsyringe_1.container.resolve(productService_1.ProductService);
        const result = await productService.findProductByBarcode(productBarcode);
        console.log("📄 Producto encontrado por codigo de barra:", result);
    }
    catch (error) {
...
runTestFindProductByBarcode().then(() => {
    console.log('Proceso de seed completo');
})
    .catch((error) => {
    console.error('Error durante el proceso de seed:', error);
});
//# sourceMappingURL=testFindProductByBarcode.js.map