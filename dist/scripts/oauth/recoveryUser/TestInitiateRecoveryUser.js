...
const runTestInitiateRecoveryUser = async () => {
    try {
        ...
        const email = "careyes.19@est.ucab.edu.ve";
        const result = await oauthService.initiateUsernameRecover({ userId: idUser, email: email });
        console.log("📄 Intento de recuperación de contraseña para recuperacion de usuario: ", result);
    }
    catch (error) {
...
runTestInitiateRecoveryUser().then(() => {
    console.log('Proceso de seed completo');
})
    .catch((error) => {
    console.error('Error durante el proceso de seed:', error);
});
//# sourceMappingURL=TestInitiateRecoveryUser.js.map