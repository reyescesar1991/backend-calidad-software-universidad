...
const runTestInactivateProduct = async () => {
    try {
        ...
        const productId = validations_1.objectIdSchema.parse("6876af6793cb0ae480ef33a2");
        const productService = tsyringe_1.container.resolve(productService_1.ProductService);
        const result = await productService.inactivateProduct(productId);
        console.log("📄 Producto inactivado:", result);
    }
    catch (error) {
...
runTestInactivateProduct().then(() => {
    console.log('Proceso de seed completo');
})
    .catch((error) => {
    console.error('Error durante el proceso de seed:', error);
});
//# sourceMappingURL=testInactivateProduct.js.map