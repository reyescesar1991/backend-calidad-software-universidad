...
const runTestGetTotalStockMonetaryValueByWarehouse = async () => {
    try {
        ...
        const warehouseId = validations_1.objectIdSchema.parse("67f690a03ad8f43e09cec544");
        const productStockService = tsyringe_1.container.resolve(productService_1.ProductStockService);
        const result = await productStockService.getTotalStockMonetaryValueByWarehouse(warehouseId);
        console.log(`📄Valor total de este almacen:`, result);
    }
    catch (error) {
...
runTestGetTotalStockMonetaryValueByWarehouse().then(() => {
    console.log('Proceso de seed completo');
})
    .catch((error) => {
    console.error('Error durante el proceso de seed:', error);
});
//# sourceMappingURL=testGetTotalStockMonetaryValueByWarehouse.js.map