...
export const validateInitiateRecoveryUser2FA = (req: Request, res: Response, next: NextFunction) => {
    logger.info('--- MIDDLEWARE: Ejecutando validateInitiateRecoveryUser2FA ---');
    ...
    try {
        ...
        logger.info('Validación de Zod para la solicitud del 2FA de recuperacion de usuario SUPERADA. Pasando al controlador.');
        next();

    } catch (error) {

        handleMiddlewareError(error, res, next, 'Datos de solicitud de 2fa de recuperacion de usuario inválidos');
    }
};